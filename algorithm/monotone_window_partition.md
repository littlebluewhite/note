...
- 只要 `max - min > k`，就左移指標 `l` 並移除越界的隊首，直到恢復合法；`l` 保持為「最左且合法」的位置，所以合法左端必然連續，是 `[l, r]`。
- 令 `dp[i]` 為切出前 `i` 個元素的合法方案數，`prefix[i] = Σ_{0..i} dp`。當右端為 `r` 時，合法左端範圍是 `l..r`，故 `dp[r+1] = prefix[r] - prefix[l-1]`（若 `l=0` 則用 0），利用前綴和 O(1) 取得區間和。
...
> 為什麼 `dp[3] = prefix[2] - prefix[0]`：當右端在 `r=2` 時，合法左端區間是 `l=1..2`，需要 `dp[1]+dp[2]`。`prefix[2]=dp[0]+dp[1]+dp[2]`，`prefix[0]=dp[0]`，相減即得到 `dp[1]+dp[2]`。

## 為什麼用 deque（單調隊列）

- 目標：在滑動視窗內 O(1) 取得最大/最小值，且整體 O(n)。
- 維護兩個 deque：`max_q` 遞減（頭是最大值索引）、`min_q` 遞增（頭是最小值索引）。
- 右端擴張 r：
  - 在 `max_q` 從尾彈掉所有 `<= nums[r]` 的索引，再推入 `r`，隊首即視窗最大值。
  - 在 `min_q` 從尾彈掉所有 `>= nums[r]` 的索引，再推入 `r`，隊首即視窗最小值。
- 若 `max - min > k`，左端 `l` 右移縮窗，若隊首索引等於舊的 `l` 就彈出，直到回到合法；`l` 因此永遠指向「最左且合法」的位置。
- 每個索引至多進出每個 deque 各一次，總時間 O(n)，取得 max/min 是 O(1)。
