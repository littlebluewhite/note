---
title: Immer Middleware and Complex Updates / Immer 中介層與複雜更新
note_type: knowledge
domain: frontend
...
prerequisites: [08_subscribe_with_selector_and_transient_updates]
---
# Immer Middleware and Complex Updates / Immer 中介層與複雜更新

## Goal

本章目標是完成「用 immer 寫複雜巢狀更新且保持可讀性」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`08_subscribe_with_selector_and_transient_updates`。
- 下一章預告：`10_nextjs_app_router_client_boundary_setup`。

## Prerequisites

- 已完成 `08_subscribe_with_selector_and_transient_updates`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch09`）。
...
### Foundation
任務：完成 `用 immer 寫複雜巢狀更新且保持可讀性` 的最小可用版本。
...
export const useProjectStore = create(
  immer<{
projects: { id: string; tasks: { id: string; done: boolean }[] }[];
toggleTask: (pid: string, tid: string) => void;
  }>((set) => ({
projects: [],
toggleTask: (pid, tid) =>
  set((s) => {
...
- [Primary Doc](https://zustand.docs.pmnd.rs/middlewares/immer)
- [Secondary Doc](https://zustand.docs.pmnd.rs/guides/updating-state)
