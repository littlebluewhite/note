---
title: State Management: Why Zustand / 狀態管理：為什麼選 Zustand
note_type: knowledge
domain: frontend
...
---
# State Management: Why Zustand / 狀態管理：為什麼選 Zustand

## Goal

本章目標是完成「理解 Zustand 在 React 19 生態的定位」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`00_series_overview`。
- 下一章預告：`02_first_store_create_and_use_store`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch01`）。
...
### Foundation
任務：完成 `理解 Zustand 在 React 19 生態的定位` 的最小可用版本。
...
- [Primary Doc](https://zustand.docs.pmnd.rs/getting-started/comparison)
- [Secondary Doc](https://react.dev/learn/managing-state)
