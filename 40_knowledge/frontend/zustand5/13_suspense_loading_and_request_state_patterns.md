---
title: Suspense, Loading, and Request State Patterns / Suspense、Loading 與請求狀態模式
note_type: knowledge
domain: frontend
...
prerequisites: [12_server_actions_and_client_store_sync]
---
# Suspense, Loading, and Request State Patterns / Suspense、Loading 與請求狀態模式

## Goal

本章目標是完成「整合 suspense 與 request state 狀態機」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`12_server_actions_and_client_store_sync`。
- 下一章預告：`14_forms_optimistic_ui_and_rollback`。

## Prerequisites

- 已完成 `12_server_actions_and_client_store_sync`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch13`）。
...
### Foundation
任務：完成 `整合 suspense 與 request state 狀態機` 的最小可用版本。
...
const useRequestStore = create<{ users: RequestState<string[]> }>(() => ({
  users: { status: 'idle' },
}));
```
...
- [Primary Doc](https://react.dev/reference/react/Suspense)
- [Secondary Doc](https://nextjs.org/docs/app/building-your-application/routing/loading-ui-and-streaming)
