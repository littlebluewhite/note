---
title: Forms, Optimistic UI, and Rollback / 表單、Optimistic UI 與回滾
note_type: knowledge
domain: frontend
...
prerequisites: [13_suspense_loading_and_request_state_patterns]
---
# Forms, Optimistic UI, and Rollback / 表單、Optimistic UI 與回滾

## Goal

本章目標是完成「實作 optimistic 更新與失敗回滾」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`13_suspense_loading_and_request_state_patterns`。
- 下一章預告：`15_error_recovery_reset_and_resilience_patterns`。

## Prerequisites

- 已完成 `13_suspense_loading_and_request_state_patterns`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch14`）。
...
### Foundation
任務：完成 `實作 optimistic 更新與失敗回滾` 的最小可用版本。
...
export const useDraftStore = create<{
  ...
  draft: '',
  prev: '',
  setDraft: (v) => set({ prev: get().draft, draft: v }),
  rollback: () => set((s) => ({ draft: s.prev })),
}));
```
...
- [Primary Doc](https://react.dev/reference/react/useOptimistic)
- [Secondary Doc](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations)
