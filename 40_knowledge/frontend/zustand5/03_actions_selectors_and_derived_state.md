---
title: Actions, Selectors, and Derived State / Actions、Selectors 與衍生狀態
note_type: knowledge
domain: frontend
...
prerequisites: [02_first_store_create_and_use_store]
---
# Actions, Selectors, and Derived State / Actions、Selectors 與衍生狀態

## Goal

本章目標是完成「設計可維護 action 與 selector，減少 UI 耦合」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`02_first_store_create_and_use_store`。
- 下一章預告：`04_slices_pattern_and_store_composition`。

## Prerequisites

- 已完成 `02_first_store_create_and_use_store`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch03`）。
...
### Foundation
任務：完成 `設計可維護 action 與 selector，減少 UI 耦合` 的最小可用版本。
...
export const selectDoneCount = (s: ReturnType<typeof useTodoStore.getState>) =>
...
- [Primary Doc](https://zustand.docs.pmnd.rs/guides/updating-state)
- [Secondary Doc](https://zustand.docs.pmnd.rs/apis/shallow)
