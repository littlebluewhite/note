---
title: Async Actions and Error Handling / 非同步 Actions 與錯誤處理
note_type: knowledge
domain: frontend
...
prerequisites: [04_slices_pattern_and_store_composition]
---
# Async Actions and Error Handling / 非同步 Actions 與錯誤處理

## Goal

本章目標是完成「建立可取消、可回復、可觀測的 async store 流程」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`04_slices_pattern_and_store_composition`。
- 下一章預告：`06_persist_middleware_and_storage_strategy`。

## Prerequisites

- 已完成 `04_slices_pattern_and_store_composition`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch05`）。
...
### Foundation
任務：完成 `建立可取消、可回復、可觀測的 async store 流程` 的最小可用版本。
...
try {
  ...
  set({ error: 'fetch_failed', loading: false });
}
  },
}));
```
...
- [Primary Doc](https://zustand.docs.pmnd.rs/guides/async-actions)
- [Secondary Doc](https://react.dev/reference/react/useTransition)
