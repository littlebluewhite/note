---
title: Devtools, Redux Middleware, and Debugging / Devtools、Redux 中介層與除錯
note_type: knowledge
domain: frontend
...
prerequisites: [06_persist_middleware_and_storage_strategy]
---
# Devtools, Redux Middleware, and Debugging / Devtools、Redux 中介層與除錯

## Goal

本章目標是完成「加上 devtools/redux middleware 提升除錯效率」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`06_persist_middleware_and_storage_strategy`。
- 下一章預告：`08_subscribe_with_selector_and_transient_updates`。

## Prerequisites

- 已完成 `06_persist_middleware_and_storage_strategy`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch07`）。
...
### Foundation
任務：完成 `加上 devtools/redux middleware 提升除錯效率` 的最小可用版本。
...
const useDebugStore = create(
  devtools(
redux((state: { count: number }, action: { type: 'inc' | 'dec' }) => {
...
useDebugStore.dispatch({ type: 'inc' });
```
...
- [Primary Doc](https://zustand.docs.pmnd.rs/middlewares/devtools)
- [Secondary Doc](https://zustand.docs.pmnd.rs/middlewares/redux)
