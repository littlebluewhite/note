---
title: First Store: create and useStore / 第一個 Store：create 與 useStore
note_type: knowledge
domain: frontend
...
prerequisites: [01_state_management_why_zustand]
---
# First Store: create and useStore / 第一個 Store：create 與 useStore

## Goal

本章目標是完成「建立第一個 store，掌握 create/useStore 與 set/get」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`01_state_management_why_zustand`。
- 下一章預告：`03_actions_selectors_and_derived_state`。

## Prerequisites

- 已完成 `01_state_management_why_zustand`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch02`）。
...
### Foundation
任務：完成 `建立第一個 store，掌握 create/useStore 與 set/get` 的最小可用版本。
...
- [Primary Doc](https://zustand.docs.pmnd.rs/apis/create)
- [Secondary Doc](https://zustand.docs.pmnd.rs/hooks/use-store)
