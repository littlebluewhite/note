---
title: subscribeWithSelector and Transient Updates / subscribeWithSelector 與暫態更新
note_type: knowledge
domain: frontend
...
prerequisites: [07_devtools_redux_middleware_and_debugging]
---
# subscribeWithSelector and Transient Updates / subscribeWithSelector 與暫態更新

## Goal

本章目標是完成「實作局部訂閱與暫態更新，降低 render 成本」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`07_devtools_redux_middleware_and_debugging`。
- 下一章預告：`09_immer_middleware_and_complex_updates`。

## Prerequisites

- 已完成 `07_devtools_redux_middleware_and_debugging`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch08`）。
...
### Foundation
任務：完成 `實作局部訂閱與暫態更新，降低 render 成本` 的最小可用版本。
...
- [Primary Doc](https://zustand.docs.pmnd.rs/middlewares/subscribe-with-selector)
