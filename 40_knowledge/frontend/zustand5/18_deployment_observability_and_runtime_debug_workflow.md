---
title: Deployment, Observability, and Runtime Debug Workflow / 部署、可觀測性與執行期除錯流程
note_type: knowledge
domain: frontend
...
prerequisites: [17_testing_store_and_ui_with_vitest_rtl]
---
# Deployment, Observability, and Runtime Debug Workflow / 部署、可觀測性與執行期除錯流程

## Goal

本章目標是完成「把 store 觀測、告警與除錯納入部署流程」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`17_testing_store_and_ui_with_vitest_rtl`。
- 下一章預告：`19_upgrade_security_and_monthly_update_playbook`。

## Prerequisites

- 已完成 `17_testing_store_and_ui_with_vitest_rtl`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch18`）。
...
### Foundation
任務：完成 `把 store 觀測、告警與除錯納入部署流程` 的最小可用版本。
...
```tsx
export function reportStoreMetrics() {
...
- [Primary Doc](https://nextjs.org/docs/app/building-your-application/deploying)
