---
title: Server Actions and Client Store Sync / Server Actions 與 Client Store 同步
note_type: knowledge
domain: frontend
...
prerequisites: [11_ssr_hydration_and_per_request_store]
---
# Server Actions and Client Store Sync / Server Actions 與 Client Store 同步

## Goal

本章目標是完成「建立 server action 與 client store 的同步協定」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`11_ssr_hydration_and_per_request_store`。
- 下一章預告：`13_suspense_loading_and_request_state_patterns`。

## Prerequisites

- 已完成 `11_ssr_hydration_and_per_request_store`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch12`）。
...
### Foundation
任務：完成 `建立 server action 與 client store 的同步協定` 的最小可用版本。
...
export async function saveProfile(input: { name: string }) {
  return { ok: true, name: input.name };
}

'use client';

export async function submit(name: string) {
  const result = await saveProfile({ name });
  useProfileStore.getState().setName(result.name);
}
```
...
- [Primary Doc](https://nextjs.org/docs/app/building-your-application/data-fetching/server-actions-and-mutations)
- [Secondary Doc](https://zustand.docs.pmnd.rs/guides/nextjs)
