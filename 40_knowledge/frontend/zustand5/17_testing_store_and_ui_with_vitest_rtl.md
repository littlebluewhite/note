---
title: Testing Store and UI with Vitest + RTL / 用 Vitest + RTL 測試 Store 與 UI
note_type: knowledge
domain: frontend
...
prerequisites: [16_performance_tuning_selectors_shallow_and_splitting]
---
# Testing Store and UI with Vitest + RTL / 用 Vitest + RTL 測試 Store 與 UI

## Goal

本章目標是完成「建立 store 單元測試與 UI 整合測試」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`16_performance_tuning_selectors_shallow_and_splitting`。
- 下一章預告：`18_deployment_observability_and_runtime_debug_workflow`。

## Prerequisites

- 已完成 `16_performance_tuning_selectors_shallow_and_splitting`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch17`）。
...
### Foundation
任務：完成 `建立 store 單元測試與 UI 整合測試` 的最小可用版本。
...
- [Primary Doc](https://vitest.dev/guide/)
- [Secondary Doc](https://testing-library.com/docs/react-testing-library/intro/)
