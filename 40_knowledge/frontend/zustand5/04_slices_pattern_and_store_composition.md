---
title: Slices Pattern and Store Composition / Slices 模式與 Store 組合
note_type: knowledge
domain: frontend
...
prerequisites: [03_actions_selectors_and_derived_state]
---
# Slices Pattern and Store Composition / Slices 模式與 Store 組合

## Goal

本章目標是完成「用 slices pattern 管理大型 store 模組化」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`03_actions_selectors_and_derived_state`。
- 下一章預告：`05_async_actions_and_error_handling`。

## Prerequisites

- 已完成 `03_actions_selectors_and_derived_state`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch04`）。
...
### Foundation
任務：完成 `用 slices pattern 管理大型 store 模組化` 的最小可用版本。
...
```tsx
type BearSlice = { bears: number; incBear: () => void };
type FishSlice = { fish: number; incFish: () => void };
type AppStore = BearSlice & FishSlice;

const createBearSlice: StateCreator<AppStore, [], [], BearSlice> = (set) => ({
  bears: 0,
  incBear: () => set((s) => ({ bears: s.bears + 1 })),
});

const createFishSlice: StateCreator<AppStore, [], [], FishSlice> = (set) => ({
  fish: 0,
  incFish: () => set((s) => ({ fish: s.fish + 1 })),
});
```
...
- [Primary Doc](https://zustand.docs.pmnd.rs/guides/slices-pattern)
- [Secondary Doc](https://zustand.docs.pmnd.rs/guides/typescript)
