---
title: Error Recovery, Reset, and Resilience Patterns / 錯誤復原、重置與韌性模式
note_type: knowledge
domain: frontend
...
prerequisites: [14_forms_optimistic_ui_and_rollback]
---
# Error Recovery, Reset, and Resilience Patterns / 錯誤復原、重置與韌性模式

## Goal

本章目標是完成「建立可復原 store 的錯誤治理流程」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`14_forms_optimistic_ui_and_rollback`。
- 下一章預告：`16_performance_tuning_selectors_shallow_and_splitting`。

## Prerequisites

- 已完成 `14_forms_optimistic_ui_and_rollback`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch15`）。
...
### Foundation
任務：完成 `建立可復原 store 的錯誤治理流程` 的最小可用版本。
...
```tsx
const RESET = { items: [], error: null } as const;

export const useSafeStore = create<{
  items: string[];
  error: string | null;
  reset: () => void;
}>((set) => ({
...
- [Primary Doc](https://zustand.docs.pmnd.rs/guides/how-to-reset-state)
- [Secondary Doc](https://nextjs.org/docs/app/building-your-application/routing/error-handling)
