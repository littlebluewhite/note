---
title: Next.js App Router Client Boundary Setup / Next.js App Router Client 邊界設定
note_type: knowledge
domain: frontend
...
prerequisites: [09_immer_middleware_and_complex_updates]
---
# Next.js App Router Client Boundary Setup / Next.js App Router Client 邊界設定

## Goal

本章目標是完成「在 App Router 正確放置 client store 邊界」並在 Next.js App Router 專案裡能安全落地。

- 銜接上一章：`09_immer_middleware_and_complex_updates`。
- 下一章預告：`11_ssr_hydration_and_per_request_store`。

## Prerequisites

- 已完成 `09_immer_middleware_and_complex_updates`。
...
1. 建立本章示範路由與檔案夾（建議 `src/app/zustand/ch10`）。
...
### Foundation
任務：完成 `在 App Router 正確放置 client store 邊界` 的最小可用版本。
...
- [Secondary Doc](https://nextjs.org/docs/app/getting-started/server-and-client-components)
